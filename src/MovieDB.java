...
class MovieDB
{
    ...
    public static void main (String [] args)
    {
        ...
        switch (args[0]) {
            case "BpTreeMap":
                desiredMap = "BpTreeMap";
                break;
            case "LinHashMap":
                desiredMap = "LinHashMap";
                break;
            case "ExtHashMap":
                desiredMap = "ExtHashMap";
                break;
            case "TreeMap":
                desiredMap = "TreeMap";
                break;
            default:
                System.out.println("Map type incorrectly specified. Proceeding using TreeMap");
                desiredMap = "TreeMap";
                break;
        }

        Table movie = new Table ("movie", "title year length genre studioName producerNo",
                                          "Integer Integer Integer String String Integer", "title", desiredMap);

        Table cinema = new Table ("cinema", "title year length genre studioName producerNo",
                                            "String Integer Integer String String Integer", "title year", desiredMap);

        Table movieStar = new Table ("movieStar", "name address gender birthdate",
                                                  "String String Character String", "name", desiredMap);

        Table starsIn = new Table ("starsIn", "movieTitle movieYear starName",
                                              "String Integer String", "movieTitle movieYear starName", desiredMap);

        Table movieExec = new Table ("movieExec", "certNo name address fee",
                                                  "Integer String String Float", "certNo", desiredMap);

        Table studio = new Table ("studio", "name address presNo",
                                            "String String Integer", "name", desiredMap);

        Comparable [] film0 = { 1, 1977, 124, "sciFi", "Fox", 12345 };
        Comparable [] film1 = { 2, 1980, 124, "sciFi", "Fox", 12345 };
        Comparable [] film2 = { 3, 1985, 200, "action", "Universal", 12125 };
        Comparable [] film3 = { 4, 1978, 100, "action", "Universal", 32355 };
        out.println ();
        movie.insert (film0);
        movie.insert (film1);
        movie.insert (film2);
...
//
        //--------------------- Table scan select
        Table s_select = movie.select(new KeyType(3));
        s_select.print();
...
//
        ...
        out.println();
        Table t_rslect = movie.rangeSelect(new KeyType(2), true, new KeyType(2), false);
        t_rslect.print();
//
